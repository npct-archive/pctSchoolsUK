...
Following the model fitting and uptake scenario modelling in [@lovelace_propensity_2016-2], we have fit an (elastic net) regularised logistic regression model (INCLUDE THE ELASTIC NET PENALY TERMS) to the school commuting data, obtaining the fits shown in (FIGURES), with a model and regression coefficients as follows
...
$$
where $y_i$ is a binary-valued response equal to 1 when the pupil performed his/her school commute by cycling, $d_i$ is the route length in kilometres, $g_i$ is the gradient of the route measured in (UNITS), and $q_i$ is the quietness diversion factor (the ratio of the length of the quiet route divided by the length of the fast route).

This model provides a baseline probability for cycling for a given route distance, gradient, and quietness diversion factor, as shown in the figures below

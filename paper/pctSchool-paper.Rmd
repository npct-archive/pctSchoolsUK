...
address:
  ...
  - code: University of Leeds
    address: Leeds Institute for Data Analytics, Level 11, Worsley Building Clarendon Way, University of Leeds, Leeds, LS2 9NL, United Kingdom 
abstract: |
  ...
  Instead, the paper investigates the potential for modal switch to cycling for the school commute, based on a case study of English data.
...
if(!file.exists("paper/figures/schools_england.pdf") | !file.exists("paper/figures/flows_england.pdf")){
  pdf(file = "paper/figures/schools_england.pdf")
  plot(las); points(schools, col="red", pch='.', cex=2)
  dev.off()
  pdf(file = "paper/figures/flows_england.pdf")
  plot(las); lines(flow, col = 'blue')
  dev.off()
}
```
!["The spatial distribution of schools in the dataset."](figures/schools_england.pdf)   !["The nationwide school commute flows containing at least 10 pupils."](figures/flows_england.pdf)

Comparison of cycling to work and cycling to school.

```{r, echo=FALSE, include=FALSE}
if(!file.exists("paper/figures/pcycle_schools.pdf") | !file.exists("paper/figures/pcycle_work.pdf")){
  #pdf(file="paper/figures/pcycle_schools.pdf")
  #qtm(lads_active, fill = "Cycle")
  #dev.off()
  
  pcycle_schools = qtm(lads_active, fill = "Cycle")
  save_tmap(pcycle_schools, "paper/figures/pcycle_schools.pdf")
  ...
  #pdf(file = "paper/figures/pcycle_work.pdf")
  #qtm(lad_pcycle, fill = "pcycle")
  #dev.off()
  pcycle_work = qtm(lad_pcycle, fill = "pcycle")
  save_tmap(pcycle_work, "paper/figures/pcycle_work.pdf")
  
  #plot(schoolvswork$Cycle, schoolvswork$pcycle, xlab="Cycling commutes to school (%)", ylab="Cycling commutes to work (%)")
}
```
![Choropleth showing the proportion of pupils cycling to school across England.](figures/pcycle_schools.pdf)   ![Choropleth showing the proportion of people cycling to work across England.](figures/pcycle_work.pdf)
...
if(!file.exists("paper/figures/active_choropleth.pdf")){
  ...
  save_tmap(active_choropleth, "paper/figures/active_choropleth.pdf")
}
```
![Cycling as a fraction of all school commutes across Local Authorities in England.](figures/active_choropleth.pdf)
...
```{r, echo=FALSE, include=FALSE}
if(!file.exists("paper/figures/routes_leeds.pdf")){
  pdf(file = "paper/figures/routes_leeds.pdf")
...
```
![The desire line flows and fastest and quiet route network-level paths for school commutes in Leeds.](figures/routes_leeds.pdf)

```{r, echo=FALSE, fig.cap="The distribution of path lengths (in metres) along the quiet and the fastest route network-level alternatives for the school commutes in the data. The mean is shown with a blue diamond, and the corresponding box plot is shown as well, with outliers displayed as red circles."}
...
```{r, echo=FALSE, fig.cap="The fraction of commuting pupils choosing to cycle to school, as a function of the commute distance (via the fastest route network-level path)."}
...
A substantial development that we have implemented is mode-specific routing algorithms for all of the desire lines covered in the study area. These reveal that the average trip distance of travel to school, were the 'fastest' cyclable route to be appropriate, would be XX km...

...
Active travel to school offers many benefits across a range of policy objectives. From a public health perspective it contributes to the physical and mental wellbeing of new generations (ref). Active travel to school also has environmental benefits, in the form of reduced motorised traffic, improved air quality and reduced greenhouse gas emissions [@singleton_gis_2014]. 
...
From an education perspective, physical and mental barriers to educational success can be reduced by physical activity, and there is evidence to suggest that active travel to school can improve student learning (ref).
...
A table summarising the input datasets, before and after filtering out non-secondary schools, is provided below.

```{r, echo=FALSE}
# df_input = readr::read_csv(
# '"Filtering","Level","N. rows","N. columns","Example var (char)","Mode","Example var (num)","Mean"
# Pre,School,9999,50,"Type of School",Secondary,Max_age,12.5
# Post,School,8888,50,"Type of School",Secondary,Max_age,15.3
# Pre,"OD pair",55555,300,"Type of School",Secondary,Max_age,15.3
# Post,School,8888,200,"Type of School",Secondary,Max_age,15.3
# '
# )
knitr::kable(res_active[1:10,])
#knitr::kable(df_input, caption = "Summary of input datasets provided by the Department for Education, before and after filtering")
```

The table below shows the top 10 Local Authorities by active transport prevalence.
```{r, echo=FALSE}
knitr::kable(res_active_la[1:10,])
```
...
```{r, echo=FALSE, fig.cap="Illustration of the current level of cycling to school nationally"}
plot(1:10)
```
...
```{r, echo=FALSE, fig.cap="Illustration of the current level of cycling to school at the desire line levels in two case study cities (Bradford and Cambridge)."}
plot(1:10)
```
